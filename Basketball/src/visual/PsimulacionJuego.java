...
public class PsimulacionJuego extends JDialog {
	...
	public PsimulacionJuego() {
		...
		btnSeleccionar.addActionListener(e -> {
			...
			if (juegoSeleccionado != null) {
				txtIdJuego.setText("JG-" + juegoSeleccionado.getId());
		...
		scrollPaneE1.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		panelE1.add(scrollPaneE1, BorderLayout.CENTER);

		panelE2 = crearPanelEquipo(605, 87, 341, 480);
		scrollPaneE2 = new JScrollPane();
		scrollPaneE2.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private void registrarAccion(String accion) {
		...
		case "3pts":
			...
			sumarPuntos(jugadorSeleccionado.getID_Equipo(), 3);
			break;
		case "2pts":
			...
			sumarPuntos(jugadorSeleccionado.getID_Equipo(), 2);
			break;
		case "1pts":
			...
			sumarPuntos(jugadorSeleccionado.getID_Equipo(), 1);
			break;
		case "Robo":
		...
		case "Tapón":
			...
			break;
		case "Asistencia":
			jugadorSeleccionado.getEstadisticas()
					.setAsistencias(jugadorSeleccionado.getEstadisticas().intgetAsistencias() + 1);
			break;
		case "Falta":
	...
	private Equipo equipoDelJugador() {
		return jugadorSeleccionado.getID_Equipo().equals(equipo1.getId()) ? equipo1 : equipo2;
	}

	private void sumarPuntos(String equipoId, int puntos) {
		System.out.println(equipoId);
		System.out.println(equipo1.getId());

		System.out.println(equipo2.getId());
		if (equipoId.equals(equipo1.getId())) {
	        puntajeEquipo1 += puntos;
	    } else if (equipoId.equals(equipo2.getId())) {
	...
	public void cargarJugadores(Equipo home, Equipo away, boolean habilitado) {
		...
		for (int i = 0; i < tableE1.getColumnCount(); i++) {
			// Si la columna es de imagen, usa otro renderer
			if (i != 3) {
				tableE1.getColumnModel().getColumn(i).setCellRenderer(centerRenderer1);
			}
		}
		...
		for (int i = 0; i < tableE2.getColumnCount(); i++) {
			// Si la columna es de imagen, usa otro renderer
			if (i != 3) {
				tableE2.getColumnModel().getColumn(i).setCellRenderer(centerRenderer2);
			}
		}
	...
	public void setJuegoSeleccionado(Juego juego) {
		...
		if (juego != null) {
			txtIdJuego.setText("JG-" + juego.getId());
